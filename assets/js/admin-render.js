...
function renderDashboard() {
  ...
  <section class="card" style="padding:18px"><div class="admin-top"><div><h3>${escapeHtml(tr("onlineSave"))}</h3><p class="muted">${escapeHtml(tr("onlineSaveHelp"))}</p></div><button class="btn primary" data-gh-save>${icon("save")} ${escapeHtml(tr("saveGithubNow"))}</button></div></section>
  <section class="card" style="padding:18px"><h3>${escapeHtml(tr("recentActivity"))}</h3><div class="admin-list">${(D.activity || []).slice(0, 10).map(a => `<div class="admin-row"><span>${escapeHtml(a.message)}</span><span class="badge">${new Date(a.date).toLocaleString(tr("all") === "All" ? "en-US" : "fr-FR")}</span></div>`).join("")}</div></section>`;
}
...
function renderCollection(name) {
  ...
  return `<div class="admin-top"><div><h2>${escapeHtml(labelFor(name))}</h2><p class="muted">${escapeHtml(tr("addEditDelete"))}</p></div><button class="btn primary" data-add="${name}">${icon("plus")} ${escapeHtml(tr("add"))}</button></div><div class="admin-list">${rows || `<div class="card admin-row"><span class="muted">${escapeHtml(tr("noContent"))}</span></div>`}</div>`;
}

function renderInfos() {
  return `<div class="admin-top"><div><h2>${escapeHtml(tr("mainInfos"))}</h2><p class="muted">${escapeHtml(tr("mainInfosHelp"))}</p></div><button class="btn primary" data-save-info>${icon("save")} ${escapeHtml(tr("save"))}</button></div>
...
function renderCv() {
  ...
  return `<div class="admin-top"><div><h2>${escapeHtml(tr("cvTab"))}</h2><p class="muted">${escapeHtml(tr("cvHelp"))}</p></div><button class="btn primary" data-save-cv>${icon("save")} ${escapeHtml(tr("saveCv"))}</button></div>
...
function renderGithub() {
  ...
  return `<div class="card" style="padding:18px"><h2>${escapeHtml(tr("githubSave"))}</h2><p class="muted">${escapeHtml(tr("githubHelp"))}</p><div class="form-grid" style="margin-top:16px">
  ...
  </div><div class="toolbar" style="margin-top:14px"><button class="btn" data-gh-config>${icon("save")} ${escapeHtml(tr("saveGithubConfig"))}</button><button class="btn primary" data-gh-save>${escapeHtml(tr("saveGithubNow"))}</button><span class="badge ${configured ? "" : "danger"}">${escapeHtml(configured ? tr("githubConfigured") : tr("tokenRequired"))}</span></div></div>`;
}
...
export function renderEditModal(collection, item = {}) {
  ...
  return `<div class="modal-backdrop open"><form class="modal" id="itemForm"><div class="admin-top"><h2>${escapeHtml(item.id ? tr("edit") : tr("add"))} ${escapeHtml(labelFor(collection))}</h2><button class="icon-btn" type="button" data-modal-close>x</button></div>
    ...
    <div class="toolbar" style="margin-top:16px"><button class="btn primary" data-save-item="${collection}" data-item-id="${item.id || ""}" type="button">${icon("save")} ${escapeHtml(tr("save"))}</button><button class="btn" type="button" data-modal-close>${escapeHtml(tr("cancel"))}</button></div>
  </form></div>`;
}
